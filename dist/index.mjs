...
async function editGeneralTopicName(name) {
    try {
...
async function editTopicIcon(topicId, iconId) {
    try {
...
bot.on('message', async (msg) => {
    ...
    if (!msg.is_topic_message || !msg.message_thread_id) {
        if (msg.text?.length === 3) {
            ...
            if (!topicInfo) {
                const response = await bot.createForumTopic(chatId, foundRoster.name);
...
async function sendXMPPMessageToTopic(from, message) {
...
xmppClient.on('message', async (message) => {
